  ...
  async onFileChange(e) {
    ...
    if (size > 104857600) {
      ...
      // @ts-ignore
        ...
        this.app.cardUpload({name, size, buffer});
      };
  ...
  render() {
    return <div>
      <section className="Bgc(indigo) container px-4 py-4 shadow text-light">
        <h2 className="h3 m-0 font-weight-light">Upload Card</h2>
      </section>
      <section className="Bgc($gray-800) container mt-2 shadow px-4 py-2 text-light">
        <input className="form-control-file w-auto mx-auto" type="file" accept=".png, .jpeg, .jpg" onChange={this.onFileChange}/>
      </section>
    </div>;
  }
}
